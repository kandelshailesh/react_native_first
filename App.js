...
import { StyleSheet,AsyncStorage,ActivityIndicator,StatusBar,FlatList,Text,Modal,Alert,TouchableHighlight ,View,TextInput,Button,Image,TouchableOpacity,Dimensions} from 'react-native';
...
clicked:false
  ...
  checklogin()
  {
    try
    {
      ...
      cache.getItem("usernames", function(err, value) {
       ...
       var username=value;
        ...
        // 'hello'
        cache.getItem("password", function(err, value) {
          if(typeof value === "undefined")
          {
            throw err;
          }
...
this.props.navigation.navigate('Waiter');
  ...
  login()
  {
       ...
       var result= fetch('http://10.0.3.2:4000/login', {
        ...
        {
          cache.setItem("username", this.state.username, function(err) {
            // key 'hello' is 'world' in cache
        });
        cache.setItem("password",this.state.password, function(err) {
          // key 'hello' is 'world' in cache
      });
        this.setState({
          username:'',
